...
public class Channels extends ChannelInitializer<SocketChannel> {

    private final Proxy proxy;

    public Channels(Proxy proxy) {
        this.proxy = proxy;
    }

    @Override
    protected void initChannel(SocketChannel mainSocketChannel) throws Exception {
        ...
        Channel channel = bootstrap.group(proxy.getWorkerGroup())
                ...
                .handler(new ChannelInitializer<SocketChannel>() {
                    @Override /* ServerChannelInitializer as Callback */
                    protected void initChannel(SocketChannel localChannel) throws Exception {
                        localChannel.pipeline()
                                .addLast(new SimpleChannelInboundHandler<ByteBuf>() {
                                    ...
                                    protected void channelRead0(ChannelHandlerContext ctx, ByteBuf input) {
                                        ...
                                        mainSocketChannel.writeAndFlush(msg);
                                    }
                                });
                    }
                })
